...
class OrderService:
    ...
    def create_expulsion_order(
        ...
        try:
            ...
            for student_id in student_ids:
                student = (
                    self.db.query(Student).filter(Student.id == student_id).first()
                )
                if student:
        ...
        try:
            ...
            for student_id in student_ids:
                ...
                if student and student.group:
                    # Находим группу того же направления, но с целевым курсом
                    # Ищем группу с похожим названием (меняем первую цифру)
                    current_group_name = student.group.name
                    target_group_name = None

                    # Формируем название целевой группы (заменяем первую цифру на to_course)
                    for i, char in enumerate(current_group_name):
                        if char.isdigit():
                            target_group_name = (
                                current_group_name[:i]
                                + str(to_course)
                                + current_group_name[i + 1 :]
                            )
                            break
                    ...
                    if target_group_name:
                        target_group = (
                            self.db.query(Group)
                            .filter(Group.name == target_group_name)
                            .first()
                        )

                    # Если группа не найдена, ищем по направлению и курсу
                    if not target_group:
                        target_group = (
                            self.db.query(Group)
                            .filter(
                                Group.direction_id == student.group.direction_id,
                                Group.course == to_course,
                            )
                            .first()
                        )
                    ...
                    if target_group:
                        ...
                        print(
                            f"[DEBUG] Transferred student {student.last_name} from {student.group.name} to {target_group.name}"
                        )
                    else:
                        print(
                            f"[WARNING] Target group not found for student {student.last_name}. Current: {current_group_name}, Target course: {to_course}"
                        )

                # Создаем связь студента с приказом
    ...
    def create_transfer_direction_order(
        ...
        try:
            ...
            return {
                "order": order,
                "transfer_direction_order": transfer_direction_order,
            }
